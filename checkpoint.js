...
for (var i=0; i < palabra.length; i++){
  if (palabra[i].length > laMasLarga.length){
    laMasLarga= palabra[i];
  }
}
return laMasLarga;
...
for (var i=0; i < arreglo.length; i++){
  if (arreglo[i] !== num){
    arrayDesiguales.push (arreglo[i])
  }
}
...
function crearClaseEmprendedor() {
  class Emprendedor {
      ...
      addMascota(mascota) {
       ...
       constructor.mascotas.push(mascota);
